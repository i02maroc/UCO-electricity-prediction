...
print("Cargando datos y aplicando Ingeniería de Variables V4 (LSTM)...")
...
print("\n" + "="*60)
print(f"INICIANDO EJECUCIÓN LSTM V4 CON GUARDADO")
...
for i, SEED in enumerate(SEEDS):
    ...
    save_path = f"../../models/v4/lstm/{SEED}/"
    ...
    model.save(f"{save_path}lstm_model_v4.keras")
    ...
    plt.title(f'LSTM V4 - Semilla {SEED} - Curva de Aprendizaje'); plt.legend(); plt.grid(True)
    ...
    plt.plot(test_dates[:168], y_pred_real[:168], label='LSTM V4', linestyle='--', color='#9467bd')
    plt.title(f'LSTM V4 - Semilla {SEED} - Predicción 1ª Semana'); plt.legend(); plt.grid(True)
    ...
    plt.plot(test_dates[:336], y_pred_real[:336], label='LSTM V4', linestyle='--', color='#9467bd')
    plt.title(f'LSTM V4 - Semilla {SEED} - Predicción 2 Semanas'); plt.legend(); plt.grid(True)
    plt.savefig(f"{save_path}prediction_2weeks_v4.png", dpi=100); plt.close()

    print(f"Iteración {i+1:02d}/20 | Semilla: {SEED:<6} | MAPE: {mape_val:.2f}% | 💾 Guardado LSTM V4")
    tf.keras.backend.clear_session()

# RESUMEN
print("\n" + "="*20)
print(f"RESULTADO GLOBAL ESTADÍSTICO LSTM V4:")
