...
# Lags explícitos (El "atajo" de memoria para la LSTM)
...
# 8. GUARDARDADO
os.makedirs(f"../../models/v4/lstm/{SEED}/", exist_ok=True)
model.save(f"../../models/v4/lstm/{SEED}/lstm_model_v4.keras")
joblib.dump(scaler_target, f"../../models/v4/lstm/{SEED}/scaler_target_lstm_v4.pkl")
joblib.dump(scaler_features, f"../../models/v4/lstm/{SEED}/scaler_features_lstm_v4.pkl")
...
print("\n" + "="*65)
print(f"{'Métrica':<10} {'LSTM v1':>10} {'LSTM v2':>10} {'LSTM v4':>10} {'Naive':>10}")
print("="*65)
...
axes[0].set_title("LSTM v4 — Curva de aprendizaje")
...
axes[1].plot(test_index, y_pred_real[:n_plot], label="LSTM v4", linestyle="--")
axes[1].plot(test_index, y_naive_real[:n_plot], label="Naive (t-168)", alpha=0.6)
axes[1].set_title("LSTM v4 — Predicción Completa del Set de Test")
...
plt.tight_layout()
plt.savefig(f"../../models/v4/lstm/{SEED}/prediction_lstm_v4.png", dpi=150)
plt.show()
