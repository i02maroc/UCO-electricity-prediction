...
print("Cargando y preparando datos (LSTM V3 - Calendario + Clima)...")
...
print("\n" + "="*60)
print(f"INICIANDO EJECUCIÓN LSTM V3 CON GUARDADO")
...
for i, SEED in enumerate(SEEDS):
    ...
    save_path = f"../../models/v3/lstm/{SEED}/"
    ...
    model.save(f"{save_path}lstm_model_v3.keras")
    joblib.dump(scaler_target, f"{save_path}scaler_target_v3.pkl")
    joblib.dump(scaler_features, f"{save_path}scaler_features_v3.pkl")
    ...
    plt.title(f'LSTM V3 - Semilla {SEED} - Curva de Aprendizaje'); plt.legend(); plt.grid(True)
    plt.savefig(f"{save_path}learning_curve_v3.png", dpi=100); plt.close()

    # GRÁFICA 2: PREDICCIÓN 1 SEMANA (168h)
    plt.figure(figsize=(12, 6))
    plt.plot(df.index[test_start_idx : test_start_idx + 168], y_test_real[:168], label='Real', color='#1f77b4')
    plt.plot(df.index[test_start_idx : test_start_idx + 168], y_pred_real[:168], label='LSTM V3', linestyle='--', color='#d62728')
    plt.title(f'LSTM V3 - Semilla {SEED} - Predicción 1ª Semana'); plt.legend(); plt.grid(True)
    plt.savefig(f"{save_path}prediction_1week_v3.png", dpi=100); plt.close()

    # GRÁFICA 3: PREDICCIÓN 2 SEMANAS (336h)
    plt.figure(figsize=(15, 6))
    plt.plot(df.index[test_start_idx : test_start_idx + 336], y_test_real[:336], label='Real', color='#1f77b4')
    plt.plot(df.index[test_start_idx : test_start_idx + 336], y_pred_real[:336], label='LSTM V3', linestyle='--', color='#d62728')
    plt.title(f'LSTM V3 - Semilla {SEED} - Predicción 2 Semanas'); plt.legend(); plt.grid(True)
    plt.savefig(f"{save_path}prediction_2weeks_v3.png", dpi=100); plt.close()

    print(f"Iteración {i+1:02d}/20 | Semilla: {SEED:<6} | MAPE: {mape_val:.2f}% | 💾 Guardado LSTM V3")
    tf.keras.backend.clear_session()

# RESUMEN
print("\n" + "="*20)
print(f"RESULTADO GLOBAL ESTADÍSTICO LSTM V3:")
