...
print("Cargando y preparando datos (V3 - Calendario + Clima)...")
...
print("\n" + "="*60)
print(f"INICIANDO EJECUCIÓN GRU V3 CON GUARDADO")
...
for i, SEED in enumerate(SEEDS):
    ...
    save_path = f"../../models/v3/gru/{SEED}/"
    ...
    model.save(f"{save_path}gru_model_v3.keras")
    ...
    plt.title(f'GRU V3 - Semilla {SEED} - Curva de Aprendizaje'); plt.legend(); plt.grid(True)
    ...
    plt.plot(df.index[test_start_idx : test_start_idx + 168], y_pred_real[:168], label='GRU V3', linestyle='--', color='#d62728')
    plt.title(f'GRU V3 - Semilla {SEED} - Predicción 1ª Semana'); plt.legend(); plt.grid(True)
    ...
    plt.plot(df.index[test_start_idx : test_start_idx + 336], y_pred_real[:336], label='GRU V3', linestyle='--', color='#d62728')
    plt.title(f'GRU V3 - Semilla {SEED} - Predicción 2 Semanas'); plt.legend(); plt.grid(True)
    plt.savefig(f"{save_path}prediction_2weeks_v3.png", dpi=100); plt.close()

    print(f"Iteración {i+1:02d}/20 | Semilla: {SEED:<6} | MAPE: {mape_val:.2f}% | 💾 Guardado V3")
    tf.keras.backend.clear_session()

# RESUMEN
print("\n" + "="*20)
print(f"RESULTADO GLOBAL ESTADÍSTICO GRU V3:")
