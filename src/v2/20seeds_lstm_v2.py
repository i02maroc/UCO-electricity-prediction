...
print("Cargando y preparando datos (LSTM V2 - Calendario Académico)...")
...
print("\n" + "="*60)
print(f"INICIANDO EJECUCIÓN LSTM V2 CON GUARDADO")
...
for i, SEED in enumerate(SEEDS):
    ...
    # Carpeta específica para LSTM v2
    save_path = f"../../models/v2/lstm/{SEED}/"
    ...
    model.save(f"{save_path}lstm_model_v2.keras")
    ...
    # 6. GENERAR Y GUARDAR GRÁFICAS POR SEPARADO
    # GRÁFICA 1: CURVA DE APRENDIZAJE
    plt.figure(figsize=(10, 5))
    plt.plot(history.history['loss'], label='Train loss')
    plt.plot(history.history['val_loss'], label='Val loss')
    plt.title(f'LSTM V2 - Semilla {SEED} - Curva de Aprendizaje')
    plt.xlabel('Época')
    ...
    plt.plot(t_idx_1, y_pred_real[:n_plot_1], label='LSTM V2', linestyle='--', color='#ff7f0e') # Naranja para LSTM
    plt.title(f'LSTM V2 - Semilla {SEED} - Predicción Detallada (1ª Semana)')
    ...
    plt.plot(t_idx_2, y_pred_real[:n_plot_2], label='LSTM V2', linestyle='--', color='#ff7f0e')
    plt.title(f'LSTM V2 - Semilla {SEED} - Predicción General (2 Semanas)')
    plt.ylabel('Energía activa (Wh)')
    plt.legend(); plt.grid(True)
    plt.tight_layout()
...
# RESUMEN FINAL LSTM V2
print("\n" + "="*20)
print(f"RESULTADO GLOBAL ESTADÍSTICO LSTM V2:")
