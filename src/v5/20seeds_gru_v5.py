...
print("\n" + "="*65)
print(f"INICIANDO EJECUCIÓN GRU V5 CON GUARDADO")
...
for i, SEED in enumerate(SEEDS):
    ...
    save_path = f"../../models/v5/gru/{SEED}/"
    ...
    model.save(f"{save_path}gru_model_v5.keras")
    ...
    plt.title(f'GRU V5 — Semilla {SEED} — Curva de Aprendizaje ({epocas_reales} épocas)')
    ...
    plt.plot(t_idx_1, y_pred_real[:n_plot_1], label='GRU V5', linestyle='--', color='#0d742f')
    plt.title(f'GRU V5 — Semilla {SEED} — Predicción 1ª Semana (MAPE: {mape_val:.2f}%)')
    ...
    plt.plot(t_idx_2, y_pred_real[:n_plot_2], label='GRU V5', linestyle='--', color='#0d742f')
    plt.title(f'GRU V5 — Semilla {SEED} — Predicción General (2 Semanas)')
    plt.ylabel('Energía activa (Wh)')
    plt.legend(); plt.grid(True)
    plt.tight_layout()
...
# RESUMEN
print("\n" + "="*65)
print(f"RESULTADO GLOBAL ESTADÍSTICO GRU V5:")
...
print("="*65)
print(f"\nCOMPARATIVA:")
print(f"  GRU V1 (referencia): {mape_v1_medio:.2f}%")
print(f"  GRU V5 (este run):   {np.mean(resultados_mape):.2f}%")
print("="*65 + "\n")