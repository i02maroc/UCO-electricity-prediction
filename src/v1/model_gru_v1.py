...
# 8. GUARDARDADO
os.makedirs(f"../../models/v1/gru/{SEED}/", exist_ok=True)
model.save(f"../../models/v1/gru/{SEED}/gru_model_v1.keras")
joblib.dump(scaler_target, f"../../models/v1/gru/{SEED}/gru_scaler_target_v1.pkl")
joblib.dump(scaler_features, f"../../models/v1/gru/{SEED}/gru_scaler_features_v1.pkl")
...
active_real = df["active_power"].values
y_naive_real = active_real[test_start_idx-168 : test_start_idx-168+len(y_test_real)]
...
mae_gru = mean_absolute_error(y_test_real, y_pred_real)
rmse_gru = np.sqrt(mean_squared_error(y_test_real, y_pred_real))
mape_gru = mape(y_test_real, y_pred_real)
...
# Resultados LSTM anteriores para comparativa
mae_lstm, rmse_lstm, mape_lstm = 56.3, 67.0, 5.23

print("\n" + "="*55)
print(f"{'Métrica':<10} {'LSTM':>10} {'GRU':>10} {'Naive':>12}")
print("="*55)
print(f"{'MAE':<10} {mae_lstm:>10.1f} {mae_gru:>10.1f} {mae_naive:>12.1f}")
print(f"{'RMSE':<10} {rmse_lstm:>10.1f} {rmse_gru:>10.1f} {rmse_naive:>12.1f}")
print(f"{'MAPE':<10} {mape_lstm:>9.2f}% {mape_gru:>9.2f}% {mape_naive:>11.2f}%")
print("="*55)
...
axes[0].set_title('GRU — Curva de aprendizaje')
...
axes[1].plot(test_index, y_pred_real[:n_plot], label='GRU', linestyle='--')
axes[1].plot(test_index, y_naive_real[:n_plot], label='Naive (t-168)', alpha=0.6)
axes[1].set_title('GRU — Predicción primera semana del test')
...
plt.tight_layout()
plt.savefig(f"../../models/v1/gru/{SEED}/prediction_gru.png", dpi=150)
plt.show()
