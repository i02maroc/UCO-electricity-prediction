...
# 8. GUARDARDADO
os.makedirs(f"../../models/v1/lstm/{SEED}/", exist_ok=True)
model.save(f"../../models/v1/lstm/{SEED}/lstm_model_v1.keras")
joblib.dump(scaler_target, f"../../models/v1/lstm/{SEED}/scaler_target_lstm_v1.pkl")
joblib.dump(scaler_features, f"../../models/v1/lstm/{SEED}/scaler_features_lstm_v1.pkl")
...
print(f"\nLSTM   — primeras 5 predicciones: {y_pred_real[:5].round(1)}")
...
mape_lstm = mape(y_test_real, y_pred_real)
...
print("\n" + "="*55)
print(f"{'Métrica':<10} {'LSTM':>12} {'Naive (t-168)':>15}")
print("="*55)
print(f"{'MAE':<10} {mae_lstm:>12.1f} {mae_naive:>15.1f}")
print(f"{'RMSE':<10} {rmse_lstm:>12.1f} {rmse_naive:>15.1f}")
print(f"{'MAPE':<10} {mape_lstm:>11.2f}% {mape_naive:>14.2f}%")
print("="*55)
...
axes[0].set_title('LSTM — Curva de aprendizaje')
...
axes[1].plot(test_index, y_pred_real[:n_plot], label='LSTM', linestyle='--')
axes[1].plot(test_index, y_naive_real[:n_plot], label='Naive (t-168)', alpha=0.6)
axes[1].set_title('LSTM — Predicción primera semana del test')
...
plt.tight_layout()
plt.savefig(f"../../models/v1/lstm/{SEED}/prediction_lstm.png", dpi=150)
plt.show()
