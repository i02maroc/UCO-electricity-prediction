...
# BUCLE DE 20 SEMILLAS CON GUARDADO DE DATOS
...
print("\n" + "="*60)
print(f"INICIANDO EJECUCIÓN LSTM V1 CON GUARDADO")
...
for i, SEED in enumerate(SEEDS):
    ...
    save_path = f"../../models/v1/lstm/{SEED}/"
    ...
    model.save(f"{save_path}lstm_model_v1.keras")
    joblib.dump(scaler_target, f"{save_path}scaler_target_v1.pkl")
    joblib.dump(scaler_features, f"{save_path}scaler_features_v1.pkl")
    ...
    plt.title(f'LSTM V1 - Semilla {SEED} - Curva de Aprendizaje')
    plt.xlabel('Época'); plt.ylabel('MAE (escalado)')
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig(f"{save_path}learning_curve_v1.png", dpi=100); plt.close()
    ...
    plt.plot(t_idx_1, y_pred_real[:n_plot_1], label='LSTM V1', linestyle='--', color='#ff7f0e')
    plt.title(f'LSTM V1 - Semilla {SEED} - Predicción 1ª Semana')
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig(f"{save_path}prediction_1week_v1.png", dpi=100); plt.close()
    ...
    plt.plot(t_idx_2, y_pred_real[:n_plot_2], label='LSTM V1', linestyle='--', color='#ff7f0e')
    plt.title(f'LSTM V1 - Semilla {SEED} - Predicción 2 Semanas')
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig(f"{save_path}prediction_2weeks_v1.png", dpi=100); plt.close()

    print(f"Iteración {i+1:02d}/20 | Semilla: {SEED:<6} | MAPE: {mape_val:.2f}% | Guardado")
    tf.keras.backend.clear_session()

# RESUMEN FINAL
print("\n" + "="*20)
print(f"RESULTADO GLOBAL ESTADÍSTICO LSTM V1:")
