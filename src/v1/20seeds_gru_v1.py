...
print("Cargando y preparando datos (GRU V1 - Baseline)...")
...
print("\n" + "="*60)
print(f"INICIANDO EJECUCIÓN GRU V1 CON GUARDADO")
...
for i, SEED in enumerate(SEEDS):
    ...
    save_path = f"../../models/v1/gru/{SEED}/"
    ...
    model.save(f"{save_path}gru_model_v1.keras")
    ...
    plt.title(f'Semilla {SEED} - Curva de Aprendizaje (MAE)')
    plt.xlabel('Época')
    plt.ylabel('MAE (escalado)')
    plt.legend(); plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_path}learning_curve_v1.png", dpi=100)
    plt.close()
    ...
    plt.plot(t_idx_1, y_pred_real[:n_plot_1], label='GRU V1', linestyle='--', color='#ff7f0e')
    plt.title(f'Semilla {SEED} - Predicción Detallada (1ª Semana)')
    plt.ylabel('Energía activa (Wh)')
    plt.legend(); plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_path}prediction_1week_v1.png", dpi=100)
    plt.close()
    ...
    plt.plot(t_idx_2, y_pred_real[:n_plot_2], label='GRU V1', linestyle='--', color='#ff7f0e')
    plt.title(f'Semilla {SEED} - Predicción General (2 Semanas)')
    plt.ylabel('Energía activa (Wh)')
    plt.legend(); plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{save_path}prediction_2weeks_v1.png", dpi=100)
    plt.close()
...
# RESUMEN
print("\n" + "="*20)
print(f"RESULTADO GLOBAL ESTADÍSTICO V1:")
